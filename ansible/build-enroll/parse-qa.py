...
for server in servers:
   ...
   ## sshusers?
   alldata[server]['sshusers'] = 'no'
   alldata[server]['ssh_as_root'] = 'yes'
   try:
       with open('%s/etc/ssh/sshd_config' % files) as sshd:
           for line in sshd:
               if re.match('allowgroups.*sshusers', line, flags=re.IGNORECASE):
                   alldata[server]['sshusers'] = 'yes'
               elif re.match('PermitRootLogin\s+no', line, flags=re.IGNORECASE):
                   alldata[server]['ssh_as_root'] = 'no'
   except:
       pass

   ## noc account
   alldata[server]['nocuser'] = 'no'
   try:
       with open('%s/etc/passwd' % files) as passwd:
           for line in passwd:
               if re.match('noc:', line, flags=re.IGNORECASE):
                   alldata[server]['nocuser'] = 'yes'
   except:
       pass

   ## ntp peers/source
   alldata[server]['ntp_peers'] = 'no'
   try:
       with open('%s/ntp_peers' % outputs) as ntp:
           for line in ntp:
               if re.match('\^\*', line):
                   alldata[server]['ntp_peers'] = 'yes'
   except:
       pass

   ## check for running processes
   alldata[server]['nimbus_running'] = 'no'
   alldata[server]['avamar_running'] = 'no'
   try:
       with open('%s/process_list' % outputs) as nimbus:
           for line in nimbus:
               if re.search('nimbus\(cdm\)', line):
                   alldata[server]['nimbus_running'] = 'yes'
               elif re.search('/usr/local/avamar/bin/avagent.bin', line):
                   alldata[server]['avamar_running'] = 'yes'
   except:
       pass
   ...
   ports = []
   try:
       with open('%s/netstat_listening' % outputs) as netstat:
           for line in netstat:
               m = re.match('tcp.*0\.0\.0\.0:(\d+).*LISTEN', line)
               if m:
                   #ports += '%s ' % m.group(1)
       ...
       for port in sorted(ports):
           listening += '%s ' % port
       alldata[server]['listening_tcp_ports'] = listening
   except:
       alldata[server]['listening_tcp_ports'] = ''
       pass

   ## syslog
   alldata[server]['syslog_to_remote'] = 'no'
   try:
       with open('%s/etc/rsyslog.d/1-cdw.conf' % files) as rsyslog:
           for line in rsyslog:
               if re.match('\*\.debug,mail\.none\s+@', line):
                   alldata[server]['syslog_to_remote'] = 'yes'
   except:
       pass
...
csv_columns = ['hostname','architecture','os','version','family','kernel','ip','netmask','gateway','interface','macaddress','model','serial','vendor','selinux','memtotal','memfree','swaptotal','swapfree','cores','processors','threads','vcpus','spacealert','sshusers','nocuser','ssh_as_root','ntp_peers','nimbus_running','avamar_running','listening_tcp_ports','syslog_to_remote']
try:

...
for srv in sorted(found):
   if os.path.isfile("%s/output/%s/setup" % (dir,srv)):
      servers.append(srv)
   else:
      skipped.append(srv)
...
for server in servers:
   ...
   ## dns_lookup output
   dns_lookup = {}
   with open('%s/dns_lookups' % outputs) as data_file:
      for line in data_file:
         m = re.split(": ", line.rstrip())
         if m:
            if len(m) > 1:
               dns_lookup[m[0]] = m[1]
            else:
   ...
   ## /etc/ntp.conf
   alldata[server]['ntpservers'] = ''
   ntplist = []
   with open('%s/etc/ntp.conf' % files) as data_file:
      for line in data_file:
         m = re.search("^server\s+(\S+)", line)
         if m:
            ntplist.append("%s" % m.group(1))
   ntplist = sorted(ntplist)
   if len(ntplist) >= 1:
      for i in xrange(0, len(ntplist)):
         if i < (len(ntplist)-1):
            if ntplist[i] in dns_lookup:
               alldata[server]['ntpservers'] += "%s (%s)\015" % (ntplist[i], dns_lookup[ntplist[i]])
            else:
               alldata[server]['ntpservers'] += "%s\015" % ntplist[i]
         else:
            if ntplist[i] in dns_lookup:
               alldata[server]['ntpservers'] += "%s (%s)" % (ntplist[i], dns_lookup[ntplist[i]])
            else:
               alldata[server]['ntpservers'] += ntplist[i]
   else:
      alldata[server]['ntpservers'] = 'No NTP servers'

   ## ntpq_peers output
   alldata[server]['ntpsync'] = 'no NTP source'
   with open('%s/ntpq_peers' % outputs) as data_file:
      for line in data_file:
         m = re.search("^\*(\S+)", line)
         if m:
            alldata[server]['ntpsync'] = m.group(1)

   ## Ansible setup
   with open('%s/setup' % outputs) as data_file:
      ...
      alldata[server]['selinux'] = data['ansible_selinux']['status']
...
csv_columns = ['hostname','architecture','os','version','family','kernel','iptables_loaded','ip','netmask','gateway','interface','macaddress','model','serial','vendor','selinux','ntpservers','ntpsync','memtotal','memfree','swaptotal','swapfree','cores','processors','threads','vcpus','spacealert']
try:
